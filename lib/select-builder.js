...
class SelectBuilder extends QueryBuilder {
  ...
  select(...fields) {
    ...
    select.push(...fields.map(this.escapeKey));
    return this;
  }
  ...
  where(field, condition, value, neg) {
  ...
  whereNot(field, cond, value) {
    this.where(field, cond, value, 'NOT');
    return this;
  }
  ...
  orderBy(key, order = 'ASC') {
    ...
    this.operations.orderBy.push({ field, dir });
    return this;
  }

  groupBy(...fields) {
    const keys = fields.map(this.escapeKey);
    this.operations.groupBy.push(...keys);
    return this;
  }
  ...
  processSelect(query) {
    const { select } = this.operations;
    if (select.length === 0) return query + ' *';
  ...
  processOrder(query) {
    const { orderBy } = this.operations;
    const order = orderBy.map(({ field, dir }) => `${field} ${dir}`);
  ...
  build() {
    ...
    const { where, groupBy, orderBy, limit, offset, from } = this.operations;
    if (from.length === 0) {
    ...
    if (orderBy.length > 0) query = this.processOrder(query);
