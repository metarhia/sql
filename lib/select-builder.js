...
class SelectBuilder extends QueryConditionsBuilder {
  ...
  // #private
  _processFrom(from, fromRaw) {
    if (fromRaw) {
      return buildSqlOrBuilder(fromRaw, this._whereValueMapper.bind(this));
    }
...
function buildSqlOrBuilder(value, valueMapper) {
  // Wrap builder but not string.
  if (typeof value === 'string') {
    return value;
  }
  const sql = valueMapper ? valueMapper(value).build() : value.build();
  return `(${sql})`;
}

function processWith(withItems, whereValueMapper) {
