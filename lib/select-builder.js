...
class SelectBuilder extends QueryBuilder {
  ...
  processWhere(query, conditions) {
    // TODO(lundibundi): support braces
    const expr = ['WHERE'];
    for (const condition of conditions) {
      if (expr.length > 1) expr.push(condition.or ? 'OR' : 'AND');
      if (condition.mod) expr.push(condition.mod);
      if (condition.key) expr.push(condition.key);
      expr.push(condition.cond);
      const par = makeParamValue(condition.cond, condition.value, this.params);
      expr.push(par);
    }
    return query + ' ' + expr.join(' ');
  }

  processOrder(query, fields) {
    const order = [...fields].map(({ field, dir }) => `${field} ${dir}`);
